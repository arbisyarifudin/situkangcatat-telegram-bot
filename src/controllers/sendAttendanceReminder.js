    ...
    topics.forEach(topic => {
      ...
      if (memberIdsWithoutAttendanceLog.length > 0) {
        ...
        memberIdsWithoutAttendanceLog.forEach(memberId => {
          const { first_name: firstName, last_name: lastName, username } = members.find(member => member.id === memberId && member.group_id === groupId)
          const memberName = `${firstName} ${lastName || ''} ${username ? `(@${username})` : ''}`
          messageText += `\n- ${memberName}`
        })
      }
